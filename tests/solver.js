...
exports.SudokuSolver = exports.Difficulty = exports.SolveStatus = exports.DIGITS = exports.UNITS = exports.WORDS = exports.CELLS = void 0;
...
(function (SolveStatus) {
    SolveStatus[SolveStatus["NO_SOLUTION"] = 0] = "NO_SOLUTION";
    SolveStatus[SolveStatus["UNIQUE"] = 1] = "UNIQUE";
    SolveStatus[SolveStatus["MULTIPLE"] = 2] = "MULTIPLE";
    SolveStatus[SolveStatus["NODE_LIMIT"] = 3] = "NODE_LIMIT";
    SolveStatus[SolveStatus["TIMEOUT"] = 4] = "TIMEOUT";
})(SolveStatus || (exports.SolveStatus = SolveStatus = {}));
var Difficulty;
(function (Difficulty) {
    Difficulty[Difficulty["EASY"] = 0] = "EASY";
    Difficulty[Difficulty["MEDIUM"] = 1] = "MEDIUM";
    Difficulty[Difficulty["HARD"] = 2] = "HARD";
    Difficulty[Difficulty["SAMURAI"] = 3] = "SAMURAI";
...
const u32 = (x) => (x >>> 0);
const u32not = (x) => ((~x) >>> 0);
const lowbit32 = (x) => u32(x & -x);
const bitIndexLow = (singleBit) => {
...
const popcount9 = (mask9) => POPCOUNT_9BIT[mask9 & 0x1FF];
/* -----------------------------
   CELL BIT HELPERS
----------------------------- */
const cellWord = (cell) => (cell >>> 5); // 0..2
const cellBit = (cell) => ((1 << (cell & 31)) >>> 0);
/* -----------------------------
   DETERMINISTIC PRNG (xorshift32)
----------------------------- */
const xorshift32 = (state) => {
    let x = state;
    x = u32(x ^ (x << 13));
    x = u32(x ^ (x >>> 17));
    x = u32(x ^ (x << 5));
...
function generateMasks() {
    const UNIT_MASK = new Uint32Array(exports.UNITS * exports.WORDS);
    const PEER_MASK = new Uint32Array(exports.CELLS * exports.WORDS);
    const CELL_TO_UNITS = new Uint8Array(exports.CELLS * 3);
    const setBitArr = (arr, base, cell) => {
    ...
    for (let r = 0; r < 9; r++) {
        const base = r * 3;
        for (let c = 0; c < 9; c++)
            setBitArr(UNIT_MASK, base, r * 9 + c);
    }
    // cols (u=9..17)
    for (let c = 0; c < 9; c++) {
        const u = 9 + c;
...
class SudokuSolver {
    constructor(masks) {
        this.ACTIVITY_DECAY = 0.95;
        this.RESCALE_THRESHOLD = 1e150;
        this.RESCALE_FACTOR = 1e-100;
    ...
    enableRandomMRVTieBreak(on) {
        this.randomMRVTieBreak = !!on;
    }
    enableRandomValueChoice(on) {
        this.randomValueChoice = !!on;
    }
    enableHeavyRules(on) {
        this.heavyEnabled = !!on;
    }
    setHeavySchedule(atRootOnly, depthLimit, dirtyUnitsOnly) {
    ...
    pushTrail(cell, oldMask) {
        if (this.trailPtr >= MAX_TRAIL)
            return false;
    ...
    adjustAssignedCount(oldMask, newMask) {
    ...
    restoreCellPlanesFromMask(cell, mask9) {
        const w = cellWord(cell);
    ...
    maybeRescaleActivities() {
        if (this.ACTIVITY_INC < this.RESCALE_THRESHOLD)
            return;
        for (let c = 0; c < 81; c++)
            this.ACTIVITY[c] = this.ACTIVITY[c] * this.RESCALE_FACTOR;
        this.ACTIVITY_INC = this.ACTIVITY_INC * this.RESCALE_FACTOR;
    }
    bumpActivity(cell) {
        this.ACTIVITY[cell] = this.ACTIVITY[cell] + this.ACTIVITY_INC;
        if (this.ACTIVITY[cell] >= this.RESCALE_THRESHOLD)
    ...
    assign(cell, digit) {
        ...
        for (let ww = 0; ww < 3; ww++) {
            ...
            while (peers !== 0) {
                ...
                if (!this.remove(p, digit))
                    return false;
            }
        }
        return true;
    }
    restrictTo(cell, keepMask9) {
        const old = this.CELL_MASK[cell];
    ...
    /* ============================================================
       ...
       ============================================================ */
    applyLockedCandidates(unitMaskBits) {
        for (let boxU = 18; boxU <= 26; boxU++) {
            if (!this.unitIsInMask(unitMaskBits, boxU))
                continue;
    ...
    /* ============================================================
       ...
       ============================================================ */
    applyHiddenPairs(unitMaskBits) {
        for (let u = 0; u < 27; u++) {
            if (this.heavyDirtyUnitsOnly && !this.unitIsInMask(unitMaskBits, u))
                continue;
            const baseU = u * 3;
            for (let d = 0; d < 9; d++) {
                ...
                this.hpCnt[d] = (popcount32(this.hpCand0[d]) + popcount32(this.hpCand1[d]) + popcount32(this.hpCand2[d]));
            }
            for (let d1 = 0; d1 < 9; d1++) {
                ...
                for (let d2 = d1 + 1; d2 < 9; d2++) {
                    if (this.hpCnt[d2] !== 2)
                        continue;
                    if (this.hpCand0[d1] !== this.hpCand0[d2])
                        continue;
                    if (this.hpCand1[d1] !== this.hpCand1[d2])
                        continue;
                    if (this.hpCand2[d1] !== this.hpCand2[d2])
                        continue;
                    ...
                    for (let ww = 0; ww < 3; ww++) {
                        ...
                        while (mm !== 0) {
                            ...
                            mm = u32(mm & (mm - 1));
                            const cell = (ww << 5) + bitIndexLow(bb);
                            if (!this.restrictTo(cell, keepMask9))
                                return false;
                        }
                    }
                }
    ...
    propagate(heavy) {
        do {
            ...
            for (let u = 0; u < 27; u++) {
                if (!this.unitIsInMask(unitsToCheck, u))
                    continue;
                this.earlyStatus = this.limitHit();
                if (this.earlyStatus !== null)
                    return true;
                ...
                for (let d = 0; d < 9; d++) {
                    ...
                    if (total === 1) {
                        let cell = -1;
                        if (m0 !== 0)
                            cell = bitIndexLow(lowbit32(m0));
                        else if (m1 !== 0)
                            cell = 32 + bitIndexLow(lowbit32(m1));
                        else
                            cell = 64 + bitIndexLow(lowbit32(m2));
                        this.hiddenSingles++;
                        if (!this.assign(cell, d))
                            return false;
                    }
                }
            }
            // locked candidates
            if (!this.applyLockedCandidates(unitsToCheck))
                return false;
            // heavy rules scheduled
            if (heavy && this.heavyEnabled) {
                if (!this.applyHiddenPairs(unitsToCheck))
                    return false;
            }
    ...
    solveInternal(stopAt) {
        ...
        if (this.decPtr > this.maxDepth)
            this.maxDepth = this.decPtr;
        while (this.decPtr > 0 && this.solutionCount < stopAt) {
            ...
            const heavyNow = (this.heavyEnabled &&
    ...
    countSolutions(stopAt = 2) {
        return this.solveWithStopAt(stopAt, false);
    }
    solveWithStopAt(stopAt, computeDifficulty) {
        this.clearStats();
        ...
        if (computeDifficulty) {
            const logicScore = this.hiddenSingles +
