...
async function run(): Promise<void> {
  ...
  const appendPuzzle = (line: string) => {
    fs.appendFileSync(OUTPUT_FILE, line, "utf8");
  };
  const logAttempt = (line: string) => {
  ...
  while (produced < TARGET_ROWS) {
    ...
    const diff =
      DIFFICULTIES[Math.floor(attempts / SYMMETRIES.length) % DIFFICULTIES.length];
    attempts++;
    ...
    let res: GeneratedPuzzle;
    try {
      res = generateSudoku(diff, sym, baseSeed);
    } catch (err) {
    ...
    if (!accepted) {
      if (attempts % LOG_EVERY === 0) {
        console.log(
          `skip rows=${produced} attempts=${attempts} seed=${baseSeed.toString(16)} sym=${sym} target=${diff} got=${res.difficulty}`
        );
      }
      continue;
    }
    ...
    if (produced % LOG_EVERY === 0) {
      console.log(
        `rows=${produced} attempts=${attempts} seed=${baseSeed.toString(16)} sym=${sym} target=${diff} got=${res.difficulty}`
      );
    }
  }
